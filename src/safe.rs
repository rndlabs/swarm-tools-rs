...
use ethers::{prelude::*, prelude::k256::ecdsa::SigningKey};
...
const FALLBACK_HANDLER_SLOT: &str = "0x6c9a6c4a39284e37ed1cf53d337577d14212a4870fb976a4366c693b939918d5";
...
impl Safe {
    ...
    pub async fn new(
        ...
        let singleton = GnosisSafeL2::new(H160::from_str(GNOSIS_SAFE_L2_ADDRESS).unwrap(), client.clone());
        let signer = SignerMiddleware::new(client.clone(), wallet.clone().with_chain_id(chain.chain_id()));
        let contract = GnosisProxyFactory::new(H160::from_str(PROXY_FACTORY_ADDRESS).unwrap(), signer.into());

        let cd = singleton.setup(
            owners,
            threshold,
            Address::zero(),
            vec![].into(),
        ...
        let call = contract.create_proxy(
        ...
        println!("Total gas cost: {} xDAI", ethers::utils::format_units(gas_limit * gas_price, "ether").unwrap());
        ...
        if let Err(e) = tx {
            ...
            if e.to_string().contains("insufficient funds") {
                println!("Transaction failed: insufficient funds. Please check your wallet balance.");
            } else {
        ...
        println!("Submitting the transaction to Gnosis Chain... https://gnosisscan.io/tx/0x{}", hex::encode(tx.tx_hash()));
        ...
        let safe_address = receipt.logs.iter().find_map(|log| {
            if log.address == H160::from_str(PROXY_FACTORY_ADDRESS).unwrap() {
                let event = contract.event::<ProxyCreationFilter>().parse_log(log.clone()).unwrap();
                Some(event.proxy)
            } else {
                None
            }
    ...
    pub async fn load(
        ...
        multicall.add_call(safe.get_storage_at(U256::from_str(FALLBACK_HANDLER_SLOT).unwrap(), U256::from(1)), false);
